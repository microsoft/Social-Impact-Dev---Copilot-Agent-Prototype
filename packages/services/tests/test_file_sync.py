...
class TestFileSyncService:
    def test_get_last_sync_no_metadata(self, sync_service, mock_blob_service):
        mock_blob_service.download_bytes.return_value = None
        assert sync_service.get_last_sync() is None

    def test_get_last_sync_with_metadata(self, sync_service, mock_blob_service):
        mock_blob_service.download_bytes.return_value = b'{"last_checked_date": "2024-01-10"}'
        assert sync_service.get_last_sync() == "2024-01-10"

    def test_get_last_sync_exception(self, sync_service, mock_blob_service):
        mock_blob_service.download_bytes.side_effect = Exception("Not found")
        assert sync_service.get_last_sync() is None

    def test_sync_single_page(self, sync_service, mock_fec_client, mock_blob_service):
        mock_response = Mock()
    ...
    def test_sync_multiple_pages(self, sync_service, mock_fec_client, mock_blob_service):
        page1_response = Mock()
    ...
    def test_sync_uses_last_sync_if_no_date_provided(
        self, sync_service, mock_fec_client, mock_blob_service
    ):
    ...
    def test_process_filing_no_file_number(self, sync_service):
        result = sync_service._process_filing({})
        assert result == 0

    def test_process_filing_with_csv_and_pdf(
        self, sync_service, mock_blob_service, mock_http_client
    ):
        mock_blob_service.exists.return_value = False
        mock_response = Mock()
    ...
    def test_full_sync_flow(
        self, sync_service, mock_fec_client, mock_blob_service, mock_http_client
    ):
        mock_blob_service.download_bytes.return_value = None
        mock_blob_service.exists.return_value = False
